function BVP = observation_case(BVP, nSen, nrep)
    %% Assign from BVP
    ...
    % Sensor connectivity
    sensor_connectivity = msh.TRIANGLES(:, 1:end - 1);
    nuSenElm = size(sensor_connectivity, 1);
    element_sensor_DOFs = zeros(nuSenElm, 6);
    element_sensor_DOFs(:, 1:2:6) = 2 * sensor_connectivity - 1;
    element_sensor_DOFs(:, 2:2:6) = 2 * sensor_connectivity;

    xSensor_surf = makeSurf(sensor_connectivity, sensorCoordinates(:, 1));
    ySensor_surf = makeSurf(sensor_connectivity, sensorCoordinates(:, 2));
    ...
    BVP_sensor.msh.nElm = nuSenElm;
    BVP_sensor.msh.nodeCoordinates = sensorCoordinates;
    BVP_sensor.msh.elementNodes = sensor_connectivity;
    BVP_sensor.msh.elementDOFs = element_sensor_DOFs;
    BVP_sensor.msh.GDOFs = GSensorDOFs;
    ...
    BVP.obs.sensor_connectivity = sensor_connectivity;
    BVP.obs.nuSenElm = nuSenElm;
    BVP.obs.sensor_force = sensor_force;
    BVP.obs.element_sensor_DOFs = element_sensor_DOFs;
    BVP.obs.xSensor_surf = xSensor_surf;
    BVP.obs.ySensor_surf = ySensor_surf;
    BVP.obs.nSen = nSen;
    BVP.obs.nrep = nrep;
